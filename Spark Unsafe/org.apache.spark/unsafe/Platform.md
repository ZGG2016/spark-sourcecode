...
public final class Platform {
  ...
  //由src往dst复制length长的一段
  public static void copyMemory(
    Object src, long srcOffset, Object dst, long dstOffset, long length) {
    // Check if dstOffset is before or after srcOffset to determine if we should copy
    // forward or backwards. This is necessary in case src and dst overlap.
    // 检查dstOffset是在srcOffset前面还是后面，来决定是向前复制，还是向后复制。
    // 在src和dst重叠的情况下，这是必要的。

    //往前复制
    if (dstOffset < srcOffset) {
      while (length > 0) {
     //UNSAFE_COPY_THRESHOLD：限制每个Unsafe#copyMemory(long, long, long)复制的字节数
        long size = Math.min(length, UNSAFE_COPY_THRESHOLD);
        _UNSAFE.copyMemory(src, srcOffset, dst, dstOffset, size);
        length -= size;
        srcOffset += size;   //复制完，往后加size长度
        dstOffset += size;
      }
    } else {  //往后复制
      srcOffset += length;
  ...
  /**
   * 限制每个Unsafe#copyMemory(long, long, long)复制的字节数，以在大量复制期间，允许安全点轮询。
