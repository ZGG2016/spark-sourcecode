...
/**
 * 获取多个块的迭代器。对于本地块，从本地块管理器获取。对于远程块，使用 BlockTransferService 获取。
 *
 * 这会创建一个迭代器，其元素是 (BlockID, InputStream) 的元组，
 * 所以调用者可以以接收的顺序，以管道的方式处理块。
 *
 * 该实现对远程获取进行了限制，这样它们就不会超过 maxBytesInFlight，从而避免使用太多内存。
 *
 ...
 *
 * @param context [[TaskContext]], used for metrics update  用于指标更新
 * @param shuffleClient [[BlockStoreClient]] for fetching remote blocks 获取远程块的ShuffleClient
 * @param blockManager [[BlockManager]] for reading local blocks 读取本地块的BlockManager
 *
 *    获取的块的列表，这些块按 BlockManagerId 分组。对每个块，可以设置大小。
 ...
 * @param streamWrapper A function to wrap the returned input stream. 封装返回的输入流
 * @param maxBytesInFlight max size (in bytes) of remote blocks to fetch at any given point.  在任意点，获取远程块的最大大小(字节)
 * @param maxReqsInFlight max number of remote requests to fetch blocks at any given point.  在任意点，获取块的请求数的最大数量
 * @param maxBlocksInFlightPerAddress max number of shuffle blocks being fetched at any given point for a given remote host:port. 在以host:port形式给出的任意点，获取shuffle块的最大数量。
 * @param maxReqSizeShuffleToMem max size (in bytes) of a request that can be shuffled to memory. 一个请求可以shuffle到内存的最大字节数。
 * @param detectCorrupt whether to detect any corruption in fetched blocks.是否检测获取块的损坏
