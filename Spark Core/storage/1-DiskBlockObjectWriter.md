...
/**
 * 一个将JVM对象直接写入磁盘文件的类。这个类允许数据被追加到已存在的块上。
 *
 * 为提高效率，它保留了跨多个提交的底层文件通道。此通道会一直开启，直到调用close()
 *
 * 在出现错误的情况下，调用者应该使用revertPartialWritesAndClose()来自动地恢复未提交的部分写操作。
 *
 * 这个类不支持并发写。 一旦writer开启，就不能再次重新开启。
 *
