...
/**
 * 在每个节点 (driver and executors) 上运行的管理器，
 * 在本地模式和远程模式下，它提供了放置和检索 blocks 到不同存储(内存、磁盘和堆外内存)中的接口。 【放置、检索块】
 *
 *
 ...
 *
 * 注意：在 BlockManager 可用前，必须先调用 initialize()
 */
private[spark] class BlockManager(
  ...
  val diskBlockManager = {
    ...
    // 仅在外部服务没有在服务我们的shuffle文件时，才执行清理。
    val deleteFilesOnStop =
      !externalShuffleServiceEnabled || executorId == SparkContext.DRIVER_IDENTIFIER
    
    // DiskBlockManager：创建、维护在逻辑块和物理磁盘位置的一个映射。
  ...
  /**
   * 获得一个可以将数据直接写入到磁盘的一个block writer。
   *
   * 块将追加到指定文件名的文件中。
   *
