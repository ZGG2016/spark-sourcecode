...
/**
 * 排序任意的数据输入缓存的一个抽象。
 * 此接口需要确定给定元素索引的sort key，以及交换元素，和将数据从一个缓冲区移动到另一个缓冲区。
 *
 ...
 *
 * 声明和实例化这个类的多个子类将防止JIT内联覆盖的方法，从而降低shuffle性能。
 *
 * @tparam K Type of the sort key of each element 每个元素的排序key的类型
 * @tparam Buffer Internal data structure used by a particular format (e.g., Array[Int]). Buffer：特定格式使用的内部数据结构
 */
