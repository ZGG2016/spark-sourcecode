 ...
 *
 * 这个类是一个内部累加器集合的包装器，
 * 这些累加器表示与一个 task 相关的指标。
 * 当任务完成时，这些累加器的本地值从 executor 发送到 driver 。
 * 然后，这些值被合并到之前在 driver 上注册的相应累加器中。
 *
 * 累加器更新也会定期(在 executor 心跳上)发送给驱动程序，并且在任务失败时出现异常。
 * TaskMetrics 对象本身不应该被发送给驱动程序。
 *
 * :: DeveloperApi ::
...
class TaskMetrics private[spark] () extends Serializable {
  ...
  /**
   * 为一个特定的 shuffle dependency ，创建一个 TempShuffleReadMetrics
   *
