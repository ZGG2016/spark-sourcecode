...
/**
 * 一个仅能追加的 map ，当空间不够时，将有序内容写入到磁盘。
 *
 * 这个 map 对数据进行了两次传递:：
 *    (1)Values 合并到 combiners ，Values 是有序的，在必要时候可以溢写到磁盘。
 *    (2)从磁盘读取的 Combiners，并合并到一起。
 *
 * :: DeveloperApi ::
...
class ExternalAppendOnlyMap[K, V, C](
 ...
 //An append-only map that keeps track of its estimated size in bytes.
    ...
    val update: (Boolean, C) => C = (hadVal, oldVal) => {
    	//有过这个值，就合并进去，否则就创建一个有初始值的Combiner
    ...
    while (entries.hasNext) {
      ...
      //estimateSize：估计集合的当前大小
      val estimatedSize = currentMap.estimateSize()

      //估计的集合大小超过了 迄今观测到的 内存map的最大大小，那就覆盖。
      if (estimatedSize > _peakMemoryUsedBytes) {
        _peakMemoryUsedBytes = estimatedSize
      }

      //判断是否溢写到了磁盘，如果是，返回true
